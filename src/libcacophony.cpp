...
extern "C" {
	...
	cacophony::platform::engine* cacophony_get_engine(const std::string& instance_key)
	{
	...
	int cacophony_compile_immediate(const char* instance_key, const char* content)
	{
		...
		catch(cacophony::platform::compilation_error  ex)
		{
			std::cerr << (_error = ex.what()) << std::endl;
	...
	int cacophony_run(const char* instance_key, float* target, float origin_x, float origin_y, float origin_z)
	{
		...
		try
		{
			_engines[instance_key].run(zacc::math::vec3<float>(origin_x, origin_y, origin_z), target);
