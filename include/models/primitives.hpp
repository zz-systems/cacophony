...
namespace cacophony { namespace models {
	...
	DISPATCHED
	inline zfloat line(const Module<branch>& source, const line_settings<zfloat>& settings, const zfloat& pos)
	{
	...
	DISPATCHED
	inline zfloat plane(const Module<branch>& source, const vec3<zfloat>& pos)
	{
		return source(zfloat(pos.x, 0, pos.y))
	}

	DISPATCHED
		inline zfloat cylinder(const Module<branch>& source, const zfloat& angle, const zfloat& height)
	{
	...
	DISPATCHED
	inline zfloat sphere(const Module<branch>& source, const zfloat& lat, const zfloat& lon)
	{
		zfloat r, _lat = lat * DEG2RAD, _lon = lon * DEG2RAD;
		vec3<zfloat> pos;
