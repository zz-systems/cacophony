...
namespace cacophony { namespace modules {
	...
	public:
		...
		zfloat lowerBound = -1.0, upperBound = 1.0;
		...
		//, const Module<branch>& a, const Module<branch>& b, const Module<branch>& controller
		zfloat operator()(const vec3<zfloat>& coords) const override
		{
			...
			if (edgeFalloff > 0.0)
			{
				...
				result = result || (mask3 && interpolate_bounds(r0, r1, cv, lowerBound, edgeFalloff));
				result = result || (mask2 && r1);
				result = result || (mask1 && interpolate_bounds(r1, r0, cv, upperBound, edgeFalloff));
				result = result || (mask0 && r0);
	...
	private:
		inline zfloat interpolate_bounds(const zfloat& from,
			const zfloat& to,
			const zfloat& control,
			const zfloat& bound, const zfloat& edgeFalloff) const
		{
