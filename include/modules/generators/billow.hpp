...
namespace cacophony { namespace modules {
	...
	{
	public:
		zfloat frequency, lacunarity, persistence;
		zint seed;
		...
		{
			zfloat value = 0.0, signal = 0.0, curPersistence = 1.0;
			vec3<zfloat> n;
			...
			for (int currentOctave = 0; currentOctave < octaves; currentOctave++) {
				...
				n = clamp_int32<zfloat>(_coords);

				// Get the coherent-noise value from the input value and add it to the
				// final result.
				signal = noisegen<branch>::gradient_coherent_3d(n, seed + currentOctave, quality);

				signal = vfmsub(2, vabs(signal), 1);
